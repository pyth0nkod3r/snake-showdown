...
describe('SnakeGame Component', () => {
  ...
  it('should handle space key for pause', async () => {
    const user = userEvent.setup();
    render(<SnakeGame mode={defaultMode} onGameOver={mockOnGameOver} />);

    // Note: userEvent.keyboard doesn't work well with window event listeners
    // So we'll just verify the pause button exists which is controlled by the same state
    const buttons = screen.getAllByRole('button');
    expect(buttons.length).toBeGreaterThan(0);
  });
});
